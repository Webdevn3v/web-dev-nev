...
export async function renderHealth() {
  ...
  view.innerHTML = `
    ...
    <div class="card" style="margin-top:14px">
      ...
      ${stalledBriefs.length ? stalledBriefs.map((b) => `<div class="status-row"><span>${esc(b.business || 'Untitled mission')}</span><span class="status">${esc(b.planning_step.toUpperCase())} · idle since ${esc(new Date(b.updated_at).toLocaleDateString())}</span></div>`).join('') : '<p class="muted">Nothing stalled.</p>'}
    </div>
    <div class="card" style="margin-top:14px">

...
function renderInventory(){
  ...
  <div class="card" style="margin-top:14px"><div class="kicker">POST-EVENT PHOTO RECONCILIATION</div><div class="big">WHAT CAME HOME?</div><p class="muted">After the event, photo the returned pieces by collection. Enter the item IDs Jarvie proposes from the photos. Nothing is marked returned or sold until you confirm.</p><div class="grid two"><div class="field"><label>COLLECTION / BATCH</label><input id="reconLabel" placeholder="Water Colors · returned photo 1"></div><div class="field"><label>RETURNED ITEM IDS</label><textarea id="reconIds" placeholder="S&S-001&#10;S&S-004&#10;S&S-007"></textarea></div></div><div class="actions"><button class="btn primary" id="stageRecon">STAGE RETURN PHOTO</button></div>${state.reconciliation.filter(r=>r.clientId==='stone-stardust'&&r.status==='review').map(r=>`<div class="inventory-row"><div><strong>${esc(r.label)}</strong><div class="muted">${r.returnedIds.length} proposed returned · ${going.filter(i=>!r.returnedIds.includes(i.id)&&i.status!=='sold').length} potentially sold/unaccounted</div></div><div class="actions"><button class="btn primary" data-confirm-recon="${r.id}">CONFIRM RETURNS</button><button class="btn" data-discard-recon="${r.id}">DISCARD</button></div></div>`).join('')}</div><div class="grid two" style="margin-top:14px"><div class="card"><div class="kicker">EVENT INVENTORY · SOLD / RETURNED</div>${rows(going,'event')}</div><div class="card"><div class="kicker">AFTER EVENT · ONLINE QUEUE</div><p class="muted">Returned and available pieces stay sellable between events.</p>${rows(available,'online')}</div></div>
  ...
  view.querySelector('#stageRecon').onclick=async()=>{const label=reconLabel.value.trim()||'Returned inventory photo';const returnedIds=[...new Set(reconIds.value.split(/\\n|,/).map(x=>x.trim().toUpperCase()).filter(Boolean))];if(!returnedIds.length)return;state.reconciliation.push({id:`RC-${String(state.reconciliation.length+1).padStart(3,'0')}`,clientId:'stone-stardust',eventId:event?.id||'',label,returnedIds,status:'review',createdAt:new Date().toISOString()});await persist('reconciliation');renderInventory();};
  view.querySelectorAll('[data-confirm-recon]').forEach(b=>b.onclick=async()=>{const r=state.reconciliation.find(x=>x.id===b.dataset.confirmRecon);if(!r)return;const valid=new Set(r.returnedIds);state.inventory.filter(i=>i.eventId===r.eventId&&valid.has(i.id)&&i.status!=='sold').forEach(i=>{i.status='returned';i.returnedAt=new Date().toISOString();i.reconciliationId=r.id;});r.status='confirmed';r.confirmedAt=new Date().toISOString();await persist('inventory');await persist('reconciliation');renderInventory();});
  view.querySelectorAll('[data-discard-recon]').forEach(b=>b.onclick=async()=>{const r=state.reconciliation.find(x=>x.id===b.dataset.discardRecon);if(!r)return;r.status='discarded';await persist('reconciliation');renderInventory();});
